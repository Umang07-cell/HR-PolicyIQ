"""
Seed script — creates 3 test users and sample payroll records.
...
for u in users:
    ...
    if not existing:
        user = User(
            ...
            location=u["location"], employee_id=u["employee_id"]
        )
        db.add(user)
        db.flush()
...
# Add sample payroll for employee
emp_id = created_ids[0]
for month in ["2026-04", "2026-05", "2026-06"]:
    existing = db.query(PayrollRecord).filter(PayrollRecord.employee_id == emp_id, PayrollRecord.month == month).first()
    if not existing:
