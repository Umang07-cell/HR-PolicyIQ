...
export default function ManagerPortal() {
  ...
  const openGrievances = grievances.filter((g) => g.status === "submitted" || g.status === "in_progress");

  return (
    <div className="p-6 max-w-7xl">
      <PageHeader
        title="Manager Portal"
        subtitle="Your team's requests, grievances, and reviews"
        action={
          <button 
            onClick={() => navigate("/")} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <h2 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2">
            ...
            {leaves.length > 0 && (
              <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-0.5 rounded-full">{leaves.length}</span>
            )}
          </h2>
          {loading ? (
            ...
            <div className="space-y-3">
              {leaves.map((l) => <ApprovalCard key={l.id} leave={l} onRefresh={load} />)}
            </div>
          )}
        </div>
        ...
        <div>
          <h2 className="text-sm font-semibold text-slate-700 mb-3 flex items-center gap-2">
            ...
            {openGrievances.length > 0 && (
              <span className="bg-red-100 text-red-700 text-xs font-bold px-2 py-0.5 rounded-full">{openGrievances.length}</span>
            )}
          </h2>
          {loading ? (
            <div className="space-y-3">{Array.from({ length: 2 }).map((_, i) => <CardSkeleton key={i} />)}</div>
            ...
            <div className="space-y-3">
              {openGrievances.slice(0, 5).map((g) => (
                <div key={g.id} className="bg-white border border-slate-200 rounded-xl p-4 hover:shadow-card-hover transition-shadow">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-sm font-semibold text-slate-800 line-clamp-1">{g.title}</p>
                      <p className="text-xs text-slate-400 mt-0.5 capitalize">{g.category?.replace(/_/g, " ") || "General"}</p>
                    </div>
